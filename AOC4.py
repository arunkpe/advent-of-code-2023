...
class Cubecount:
    ...
    def power_minimum_games(self, input_balls):
        possible_games = []

        for game, max_balls in self.games.items():
            power_max_balls = [int(value) for key, value in max_balls.items()]
            final_val = 1
            for ball in power_max_balls:
                final_val = ball * final_val

            possible_games.append(final_val)
